...
| Miembro	 | Tipo |
|--------|------|
| [ewsUrl](#ewsurl-string) | Miembro	 |
| [restUrl](#resturl-string) | Miembro	 |
...
| [convertToLocalClientTime](#converttolocalclienttimetimevalue--localclienttimejavascriptapioutlook17officelocalclienttime) | Método |
...
La aplicación debe tener el permiso **ReadItem** especificado en su manifiesto para poder llamar al miembro `restUrl` en el modo lectura.

En el modo de redacción debe llamar al método [`saveAsync`](Office.context.mailbox.item.md#saveasyncoptions-callback) antes de poder usar el miembro `restUrl`. Su aplicación debe tener permisos **ReadWriteItem** para llamar al método `saveAsync`.
...
Actualmente, los tipos de eventos admitidos son `Office.EventType.ItemChanged` y `Office.EventType.OfficeThemeChanged`.
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook_1_7/office.mailboxenums.restversion)|Un valor que indica la versión de la API de REST de Outlook que se usa para recuperar el identificador de elemento.|
...
Tipo: [LocalClientTime](/javascript/api/outlook_1_7/office.LocalClientTime)
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook_1_7/office.mailboxenums.restversion)|Un valor que indica la versión de la API de REST de Outlook con la que se usará el identificador convertido.|
...
|Nombre| Tipo| Descripción|
|---|---|---|
|`input`| [LocalClientTime](/javascript/api/outlook_1_7/office.LocalClientTime)|El valor de la hora local para convertir.|
...
| `parameters.requiredAttendees` | Matriz.&lt;Cadena&gt; &#124; Matriz.&lt;[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)&gt; | Una matriz de cadenas que contiene las direcciones de correo electrónico o una matriz que contiene un objeto `EmailAddressDetails` para cada uno de los asistentes necesarios de la cita. La matriz está limitada a un máximo de 100 entradas. |
| `parameters.optionalAttendees` | Matriz.&lt;Cadena&gt; &#124; Matriz.&lt;[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)&gt; | Una matriz de cadenas que contiene las direcciones de correo electrónico o una matriz que contiene un objeto `EmailAddressDetails` para cada uno de los asistentes opcionales de la cita. La matriz está limitada a un máximo de 100 entradas. |
...
El método `displayNewMessageForm` abre un formulario que permite al usuario crear un mensaje nuevo. Si se especifican parámetros, los campos de formulario de mensaje se rellenan automáticamente con el contenido de los parámetros.
...
|Nombre| Tipo| Descripción|
|---|---|---|
| `parameters` | Objeto | Diccionario de parámetros que describen el mensaje nuevo. |
| `parameters.toRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)&gt; | Matriz de cadenas que contiene las direcciones de correo electrónico o una matriz que contiene un objeto `EmailAddressDetails` para cada uno de los destinatarios de la línea Para. La matriz está limitada a un máximo de 100 entradas. |
| `parameters.ccRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)&gt; | Matriz de cadenas que contiene las direcciones de correo electrónico o una matriz que contiene un objeto `EmailAddressDetails` para cada uno de los destinatarios de la línea CC. La matriz está limitada a un máximo de 100 entradas. |
| `parameters.bccRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook_1_7/office.emailaddressdetails)&gt; | Matriz de cadenas que contiene las direcciones de correo electrónico o una matriz que contiene un objeto `EmailAddressDetails` para cada uno de los destinatarios de la línea CCO. La matriz está limitada a un máximo de 100 entradas. |
| `parameters.subject` | String | Cadena que contiene el asunto del mensaje. La cadena está limitada a un máximo de 255 caracteres. |
| `parameters.htmlBody` | String | Cuerpo HTML del mensaje. El contenido del cuerpo está limitado a un tamaño máximo de 32 KB. |
| `parameters.attachments` | Array.&lt;Object&gt; | Matriz de objetos JSON que son elementos o archivos adjuntos. |
| `parameters.attachments.type` | String | Indica el tipo de datos adjuntos. Debe ser `file` para un archivo adjunto o `item` para un elemento adjunto. |
| `parameters.attachments.name` | String | Una cadena que contiene el nombre de los datos adjuntos, hasta 255 caracteres de longitud.|
| `parameters.attachments.url` | String | Solo se usa si `type` se establece en `file`. El URI de la ubicación del archivo. |
| `parameters.attachments.isInline` | Booleano | Solo se usa si `type` se establece en `file`. Si `true`, indica que los datos adjuntos se mostrarán en línea en el cuerpo del mensaje, y no deben mostrarse en la lista de datos adjuntos. |
| `parameters.attachments.itemId` | String | Solo se usa si `type` se establece en `item`. El identificador de elemento EWS del correo electrónico existente que desea adjuntar al mensaje de nuevo. Se trata de una cadena de hasta 100 caracteres. |
...
> [!NOTE]
> Se recomienda que los complementos de usar las API de REST en lugar de los servicios Web Exchange siempre que sea posible. 

**Tokens de REST**

Cuando se solicita un token de REST (`options.isRest = true`), el token resultante no funcionará para autenticar las llamadas a los servicios Web Exchange. El token se limitará en su ámbito para el acceso de solo lectura al elemento actual y a sus datos adjuntos, a no ser que el complemento haya especificado el permiso [`ReadWriteMailbox`](https://docs.microsoft.com/outlook/add-ins/understanding-outlook-add-in-permissions#readwritemailbox-permission) en su manifiesto. Si está especificado el permiso `ReadWriteMailbox`, el token resultante concederá acceso de lectura o escritura al correo, calendario y contactos, incluida la capacidad de enviar correo.
...
Cuando se solicita un token EWS (`options.isRest = false`), el token resultante no funcionará para autenticar las llamadas a las API de REST. El token estará limitado en su ámbito para el acceso al elemento actual.
...
|---|---|---|---|
| `options` | Objeto | &lt;optional&gt; | Un objeto literal que contiene una o más de las siguientes propiedades. |
| `options.isRest` | Booleano |  &lt;optional&gt; | Determina si el token proporcionado se usará para las API de REST de Outlook o para los servicios Web Exchange. El valor predeterminado es `false`. |
| `options.asyncContext` | Objeto |  &lt;optional&gt; | Cualquier dato de estado que se pasa al método asincrónico. |
